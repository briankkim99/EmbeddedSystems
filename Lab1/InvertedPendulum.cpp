...
double error_prev = 0;		// Previous error
 ...
 */
 ...
 // Error of the system is the angle w.r.t the vertical.
 double error = theta;

 // Find the rate of which the error is changing
 double error_derivative = error- error_prev;

 // k1 is the inital force that will be applied to the system to counteract the
 // horizontal force created by the pendulum's gravitational force
 double k1;
 k1 = (m1*g*0.5)*(m+m1);

 // k2 is the force that will be removed from k1*error to compensate for the error
 double k2;
 k2 = (m+m1);

 F = k1*error + k2*error_derivative;
