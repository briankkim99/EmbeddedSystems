...
inline void fasterGetPixel(unsigned char *image, int x, int y, int sx, unsigned char *R, unsigned char *G, unsigned char *B);
inline void fasterSetPixel(unsigned char *image, int x, int y, int sx, unsigned char R, unsigned char G, unsigned char B);
...
unsigned char *fast_rescaleImage(unsigned char *src, int src_x, int src_y, int dest_x, int dest_y)
{
 ...
 for (int y=0;y<dest_y;y++) {			// Loop over destination image
  ...
  double fy;				// Corresponding coordinates on source image
  double dy;				// Fractional component of source image coordinates

  fy=y*step_y;
  dy=fy-(int)fy;

  fasterGetPixel(src,0,floor(fy),src_x,&R1,&G1,&B1); // get N1 colours
  fasterGetPixel(src,0,ceil(fy),src_x,&R3,&G3,&B3);  // get N3 colours

  fasterGetPixel(src,0,floor(fy),src_x,&R2,&G2,&B2);	// get N2 colours
  fasterGetPixel(src,0,ceil(fy),src_x,&R4,&G4,&B4);	// get N4 colours

  // Interpolate to get T1 and T2 colours

  // Obtain final colour by interpolating between T1 and T2
  R=(unsigned char)(dy*(R2-R1)+R1);
  G=(unsigned char)(dy*(G2-G1)+G1);
  B=(unsigned char)(dy*(B2-B1)+B1);
  // Store the final colour
  fasterSetPixel(dst,0,y,dest_x,R,G,B);
  ...
  double RT2, GT2, BT2;
   ...
   fasterGetPixel(src,ceil(fx),floor(fy),src_x,&R2,&G2,&B2);	// get N2 colours
   fasterGetPixel(src,ceil(fx),ceil(fy),src_x,&R4,&G4,&B4);	// get N4 colours

   // Interpolate to get T1 and T2 colours


   if (R2 != R1) RT1=dx*(R2-R1)+R1;
   else RT1 = R1;

   if (G2 != G1) GT1=dx*(G2-G1)+G1;
   else GT1 = G1;

   if (B2 != B1) BT1=dx*(B2-B1)+B1;
   else BT1 = B1;

   if (R4 != R3) RT2=dx*(R4-R3)+R3;
   else RT2 = R3;

   if (G4 != G3) GT2=dx*(G4-G3)+G3;
   else GT2 = G3;

   if (B4 != B3) BT2=dx*(B4-B3)+B3;
   else BT2 = B3;
   ...
   // Store the final colour
   fasterSetPixel(dst,x,y,dest_x,R,G,B);
  }
 }
 return(dst);
}
 ...
 */
 ...
 for (x=0;x<dest_x;x++)			// Loop over destination image
  for (y=0;y<dest_y;y++)
  {
   double fx=x*step_x;			// Corresponding coordinates on source image
   double fy=y*step_y;
   double dx=fx-(int)fx;		// Fractional component of source image coordinates
   double dy=fy-(int)fy;   
   ...
   BT2=(dx*B4)+(1-dx)*B3;
   // Obtain final colour by interpolating between T1 and T2
   unsigned char R=(unsigned char)((dy*RT2)+((1-dy)*RT1));	// Final colour at a destination pixel
