 ...
 */
 ...
 //        list of particles.
 ***************************************************************/
 // Initialize the head of the robot.
 list = initRobot(map, sx, sy);
 // Let headParticle to represent the current particle.
 struct particle *currParticle = list;
 // To create (n - 1) particles, and let the newly created particle at the current iteration be the
 // ->next of the particle that is created at the previous iteration. 
 for(int i = 0; i <n_particles - 1; i++) {
  struct particle *nextParticle = initRobot(map, sx, sy);
  if (hit(nextParticle, map, sx, sy)) {
   nextParticle = initRobot(map, sx, sy);
  } else {
 ...
 ****************************************************************/
  // Loop over all of the measurements of each particle.
  for(int i = 0; i < 16; i++) {
   // Compute error_i of the current particle.
   double error_i = p->measureD[i] - rob->measureD[i];
   // Call GaussEval to compute the prob of the current measurement.
   double currMeasure = GaussEval(error_i, noise_sigma);
   // Add log(currMeasure) to currentParticle's probability.
   p->prob = p->prob + log(currMeasure);
   fprintf(stderr, "%f \n", p->prob);
  }
}
 ...
 */
   ...
   ******************************************************************/
   ...
   int randMoveUnits = rand() % 5;
   // Move the robot
   // If the robot, hits a wall, we make it bounce back
   if(hit(robot, map, sx, sy)) robot->theta += 180;
   move(robot, randMoveUnits);
   // Move all of the particles.
   struct particle *currParticle = list;
   for(int i = 0; i < n_particles; i++) {
    if(hit(currParticle, map, sx, sy)) currParticle->theta += 180;
    move(currParticle, randMoveUnits);
    // Compute the groundTruth;
    ground_truth(currParticle, map, sx, sy);
   ...
   //        should be brightest.
   *******************************************************************/
   // Let currParticle points to the head.
   currParticle = list;
   // intialize a double for recording the sum of all the probalilites.
   double probSum = 0; 
   // Compute the likelihood of every particle.
   for(int i = 0; i < n_particles; i++) {
     computeLikelihood(currParticle, robot,  20);
     currParticle->prob = pow(M_E, currParticle->prob);
     probSum += currParticle->prob;
     currParticle = currParticle->next;
   }
   // Create a filter array
   double filterArr[n_particles + 1];
   filterArr[0] = 0;
   // Let currParticle points to the list;
   currParticle = list;

   // Normalize the probability to ensure the sum of the all to be 1.
   for(int i = 0; i < n_particles; i++) {
    currParticle->prob = currParticle->prob / probSum;
    filterArr[i+1] = filterArr[i] + currParticle->prob * n_particles;
   ...
   *******************************************************************/
   // Struct the head
   struct particle *newHead = resample(filterArr);
   struct particle  *currNewParticle = newHead;
   // Build the linked list of the new particle.
   for(int i = 1; i < n_particles; i++) {
    struct particle *newParticle = resample(filterArr);

    // LET currParticle->next points to the newParticle.
    currNewParticle->next = newParticle;
    // Let currParticle equals to its next ;
    currNewParticle = currNewParticle->next;
   }

   struct particle *temp = list;
   while(temp->next != NULL) {
     struct particle *temp1 = temp;
     temp = temp->next;
...
struct particle *resample(double filterArr[]) {
  struct particle *newParticle = (particle*)malloc(sizeof(particle));
  // Draw a random number from 0 to n_particles
  int filterNum = rand()%n_particles;
  struct particle *currParticle = list;
  // Loop over the filterarray, terminate until the filter number is in between of two adjecent filterArrs.
  for(int i = 0; i < n_particles; i++) {
   if(filterNum >= filterArr[i] && filterNum <= filterArr[i+1]) {
      newParticle->x = currParticle->x; 
     newParticle->y = currParticle->y;
     newParticle->theta = currParticle->theta;
     newParticle->prob = currParticle->prob;
     //newParticle = currParticle;
     break;
   }
   // Let currParticle to be the next.
   currParticle = currParticle->next;
  }
  return newParticle;
}
